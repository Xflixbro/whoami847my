...
# Add these methods to your database.py file (Mehedi class)
async def total_users_count(self):
    """Count total users in database"""
    return await self.user_data.count_documents({})

async def total_files_count(self):
    """Count total files in database channel"""
    # This requires access to your channel's message count
    # You might need to implement a different approach depending on your setup
    try:
        channel = await self.client.get_chat(CHANNEL_ID)
        return channel.messages_count if hasattr(channel, 'messages_count') else "N/A"
    except:
        return "N/A"
...
async def total_storage_used(self):
    ...
    # You might need to sum up file sizes from your database
