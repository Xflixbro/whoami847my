...
async def link_menu(client: Client, message: Message):
    ...
    menu_text = to_small_caps_with_html(
        "<b>━━━━━━━━━━━━━━━━━━</b>\n"
        "<blockquote><b>ɪꜰ ᴜ ᴡᴀɴᴛ ᴛᴏ ᴘᴏꜱᴛ ꜰɪʟᴇꜱ ᴛʜᴇɴ ᴜꜱᴇ ᴛʜᴏꜱᴇ ʙᴜᴛᴛᴏɴs ᴀᴄᴄᴏʀᴅɪɴɢ ᴛᴏ ʏᴏᴜʀ ɴᴇᴇᴅ.</b></blockquote>\n"
        "<b>━━━━━━━━━━━━━━━━━━</b>"
    )
    ...
    try:
        # Send the menu with a random image (no effect to avoid EFFECT_ID_INVALID)
...
async def link_callback(client: Client, callback: CallbackQuery):
    ...
    if user_id not in admin_ids and user_id != OWNER_ID:
        try:
            ...
            logger.warning(f"QueryIdInvalid error for callback {data} by user {user_id}")
        return
    ...
    try:
        if data == "link_batch":
            ...
            await flink_command(client, callback.message, user_id=user_id)
...
async def batch(client: Client, message: Message):
    ...
    chat_id = message.chat.id
    logger.info(f"Batch command triggered by user {message.from_user.id}")
    ...
    text = to_small_caps_with_html(
        "<b>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
        "<blockquote><b>ʙᴀᴛᴄʜ ʟɪɴᴋ ɢᴇɴᴇʀᴀᴛᴏʀ</b></blockquote>\n\n"
        "<blockquote><b>ᴄʟɪᴄᴋ 'ꜱᴛᴀʀᴛ ᴘʀᴏᴄᴇꜱꜱ' ᴛᴏ ʙᴇɢɪɴ ɢᴇɴᴇʀᴀᴛɪɴɢ ᴀ ʙᴀᴛᴄʜ ʟɪɴᴋ.</b></blockquote>\n"
        "<b>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</b>"
    )
    buttons = InlineKeyboardMarkup([
...
async def batch_start_process_callback(client: Client, query: CallbackQuery):
    ...
    logger.info(f"Batch start process callback triggered by user {query.from_user.id}")
    ...
    await query.message.edit_text(
        to_small_caps_with_html(
            "<b>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
            "<blockquote><b>ꜰᴏʀᴡᴀʀᴅ ᴛʜᴇ ꜖ɪʀꜱᴛ ᴍᴇꜱꜱᴀɢᴇ ꜖ʀᴏᴍ ᴅʙ ᴄʜᴀɴɴᴇʟ (ᴡɪᴛʜ Qᴜᴏᴛᴇꜱ).</b></blockquote>\n"
            "<blockquote><b>ᴏʀ ꜱᴇɴᴅ ᴛʜᴇ ᴅʙ ᴄʜᴀɴɴᴇʟ ᴘᴏꜱᴛ ʟɪɴᴋ</b></blockquote>\n"
            "<b>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</b>"
        ),
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔴🔴 Cancel", callback_data="batch_cancel_process")]
        ]),
        parse_mode=ParseMode.HTML
    )
    await query.answer(to_small_caps_with_html("🔴"))

@Bot.on_callback_query(filters.regex(r"^batch_(cancel_process|close)$"))
async def batch_handle_back_buttons(client: Client, callback: CallbackQuery):
    ...
    if action == "cancel_process":
        ...
        await callback.answer(to_small_caps_with_html("Process cancelled"))
    elif action == "close":
        if chat_id in batch_user_data:
            del batch_user_data[chat_id]
        await callback:
...
async def handle_batch_input(client: Client, message: Message):
    ...
    chat_id = message.chat.id
    state = batch_user_data.get('state'),
    ...
    try:
        if state == 'awaiting_first_message':
            ...
            if not s_msg_id:
                await message.reply(
                    to_small_caps_with_html><b>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</b>\n<b>❌</b> Error: This forwarded post is not from my db channel or this link is not valid</b>\n</blockquote>\n<b>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</b></html>,
                    quote=True,
                    parse_mode=ParseMode.HTML
                )
                return
...
async def flink_command(client: Client, message: Message, user_id: int = None):
    ...
    logger.info(to_small_caps_with_html(f"Flink command triggered by user {effective_user_id}"))
    try:
        admin_ids = await db.get_all_admins() or []
        if effective_user_id not in admin_ids and effective_user_id != OWNER_ID:
            await message.reply_text(to_small_caps_with_html("<b>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</b>\n<b>❌ You are not authorized!</b>\n<b>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</b>"), parse_mode=ParseMode.HTML)
            return

        flink_user_data[effective_user_id] = {
            'format': None,
            'links': {},
            'edit_data': {},
            'menu_message': None,
...
async def show_flink_main_menu(client: Client, message: Message, edit: bool = False):
    ...
    try:
        ...
        buttons = [
            [
                InlineKeyboardButton("• ꜱᴇᴛ ꜰᴏʀᴍᴀᴛ •", callback_data="flink_set_format"),
                InlineKeyboardButton("• ꜱᴛᴀʀᴛ ᴘʀᴏᴄᴇꜱꜱ •", callback_data="flink_start_process")
            ],
            [
                InlineKeyboardButton("• ʀᴇꜰʀᴇꜱʜ •", callback_data="flink_refresh"),
                InlineKeyboardButton("• ᴄʟᴏꜱᴇ •", callback_data="flink_close")
            ]
        ]
        ...
        if edit:
            if message.text != text or message.reply_markup != InlineKeyboardMarkup(buttons):
                ...
                logger.info(to_small_caps_with_html(f"Skipping edit in show_flink_main_menu for user {user_id} - content unchanged"))
        else:
            msg = await message.reply(
                text=text,
                reply_markup=InlineKeyboardMarkup(buttons),
