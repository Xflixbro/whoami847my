...
async def batch(client: Client, message: Message):
    ...
    # দুটি মেসেজ পাওয়ার পর লিঙ্ক জেনারেট করা
...
async def handle_db_post_input(client: Client, message: Message):
    ...
    try:
        ...
        for part in format_parts:
            ...
            valid_files = []
            temp_id = current_id
            max_attempts = 100  # সর্বোচ্চ কতগুলো আইডি চেক করবে
            attempts = 0
            
            # প্রয়োজনীয় সংখ্যক ভ্যালিড ফাইল সংগ্রহ করা
            while len(valid_files) < count and attempts < max_attempts:
                try:
                    ...
                    logger.error(to_small_caps_with_html(f"error fetching message {temp_id}: {e}"))
                    skipped_ids.append(temp_id)
                    temp_id += 1
                    attempts += 1
            ...
            if len(valid_files) < count:
                await message.reply(
                    to_small_caps_with_html(f"<b>━━━━━━━━━━━━━━━━━━</b>\n<blockquote><b>❌ Not enough valid media files for {quality}. Required: {count}, Found: {len(valid_files)}.</b></blockquote>\n<b>━━━━━━━━━━━━━━━━━━</b>"),
                    parse_mode=ParseMode.HTML
                )
                return
        ...
        # স্কিপ করা আইডিগুলো সম্পর্কে ইউজারকে জানানো
        if skipped_ids:
            await message.reply(
                to_small_caps_with_html(f"<b>━━━━━━━━━━━━━━━━━━</b>\n<blockquote><b>⚠️ Skipped non-media message IDs: {', '.join(map(str, skipped_ids))}</b></blockquote>\n<b>━━━━━━━━━━━━━━━━━━</b>"),
                parse_mode=ParseMode.HTML
            )
