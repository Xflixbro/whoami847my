...
async def link_menu(client: Client, message: Message):
    ...
    menu_text = to_small_caps_with_html(
        "<b>━━━━━━━━━━━━━━━━━━</b>\n"
        "<blockquote><b>Choose an option to generate links for files.</b></blockquote>\n"
        "<b>━━━━━━━━━━━━━━━━━━</b>"
    )
...
async def link_callback(client: Client, callback: CallbackQuery):
    ...
    try:
        ...
        logger.error(f"Error in link_callback for user {user_id}: {e}")
...
async def batch(client: Client, message: Message):
    ...
    try:
        ...
        logger.error(f"Error sending batch menu for user {user_id}: {e}")
        await message.reply_text(
            to_small_caps_with_html("<b>━━━━━━━━━━━━━━━━━━</b>\n<b>❌ Failed to start batch process.</b>\n<b>━━━━━━━━━━━━━━━━━━</b>"),
            parse_mode=ParseMode.HTML
        )
...
async def batch_start_process_callback(client: Client, query: CallbackQuery):
    ...
    try:
        ...
        logger.error(f"Error starting batch process for user {user_id}: {e}")
        await query.message.edit_text(
            to_small_caps_with_html("<b>━━━━━━━━━━━━━━━━━━</b>\n<b>❌ Failed to start batch process.</b>\n<b>━━━━━━━━━━━━━━━━━━</b>"),
            parse_mode=ParseMode.HTML
        )
...
async def batch_handle_cancel_close(client: Client, query: CallbackQuery):
    ...
    try:
        ...
        logger.error(f"Error in batch {action} for user {user_id}: {e}")
        await query.message.edit_text(
...
async def show_flink_main_menu(client: Client, message: Message, edit: bool = False):
    ...
    user_id = message.from_user.id
    try:
        current_format = flink_user_data[user_id].get('format', "Not set")
        text = to_small_caps_with_html(
...
async def flink_set_format_callback(client: Client, query: CallbackQuery):
    ...
    user_id = query.from_user.id
    logger.info(f"Flink_set_format callback triggered by user {user_id}")
    ...
    try:
        ...
        await query.message.edit_text(
            to_small_caps_with_html(
                ...
                "<blockquote><code>360p = 2, 720p = 2, 1080p = 2</code></blockquote>\n\n"
                ...
                "<b>Send the format in the next message.</b>\n"
                "<b>━━━━━━━━━━━━━━━━━━</b>"
            ),
            reply_markup=InlineKeyboardMarkup([
...
@Bot.on_message(filters.private & filters.text & filters.regex(r"^[a-zA-Z0-9]+\s*=\s*\d+(,\s*[a-zA-Z0-9]+\s*=\s*\d+)*\s*$"))
...
async def flink_start_process_callback(client: Client, query: CallbackQuery):
    ...
    user_id = query.from_user.id
    logger.info(f"Flink_start_process callback triggered by user {user_id}")
    ...
    try:
        ...
        await query.message.edit_text(
            to_small_caps_with_html(
                ...
                "<b>Send the link or forwarded message in the next message.</b>\n"
                "<b>━━━━━━━━━━━━━━━━━━</b>"
            ),
            reply_markup=InlineKeyboardMarkup([
            ...
            for quality in quality_list[i:i+2]:
                row.append(
                    InlineKeyboardButton(
                        f"🦋 {quality} 🦋",
                        url=await create_link(client, links[quality])
                    )
                )
            buttons.append(row)
...
async def flink_edit_output_callback(client: Client, query: CallbackQuery):
    ...
    user_id = query.from_user.id
    logger.info(f"Flink_edit_output callback triggered by user {user_id}")
...
async def flink_add_image_callback(client: Client, query: CallbackQuery):
    ...
    user_id = query.from_user.id
    logger.info(f"Flink_add_image callback triggered by user {user_id}")
...
async def flink_add_caption_callback(client: Client, query: CallbackQuery):
    ...
    user_id = query.from_user.id
    logger.info(f"Flink_add_caption callback triggered by user {user_id}")
...
async def flink_done_output_callback(client: Client, query: CallbackQuery):
    ...
    user_id = query.from_user.id
    logger.info(f"Flink_done_output callback triggered by user {user_id}")
...
async def flink_refresh_callback(client: Client, query: CallbackQuery):
    ...
    user_id = query.from_user.id
    logger.info(f"Flink_refresh callback triggered by user {user_id}")
