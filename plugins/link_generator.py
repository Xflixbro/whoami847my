...
async def link_generator(client: Client, message: Message):
    ...
    while True:
        try:
            ...
            logger.error(f"Timeout error waiting for message in genlink command for user {chat_id}")
            await message.reply_text(
                to_small_caps_with_html("<b>━━━━━━━━━━━━━━━━━━</b>\n<b>❌ Timeout: No response received.</b>\n<b>━━━━━━━━━━━━━━━━━━</b>"),
                parse_mode=ParseMode.HTML
            )
...
async def custom_batch(client: Client, message: Message):
    ...
    while True:
        ...
        try:
            ...
            logger.error(f"Error storing message in custom_batch for user {chat_id}: {e}")
            continue
...
async def flink_command(client: Client, message: Message):
    ...
    try:
        admin_ids = await db.get_all_admins() or []
        logger.info(f"Admin check for flink command: user_id={user_id}, admins={admin_ids}, OWNER_ID={OWNER_ID}")
        if user_id not in admin_ids and user_id != OWNER_ID:
            await message.reply_text(
