...
async def show_force_sub_settings(client: Client, chat_id: int, message_id: int = None):
    ...
    settings_text += f"<blockquote><b>Global Force-Sub Mode: {'🟢 Enabled' if global_mode else '🔴 Disabled'}</b></blockquote>\n\n"
    ...
    if not channels:
        ...
        for ch_id in channels:
            try:
                ...
                settings_text += f"<blockquote><b><a href='{link}'>{chat.title}</a> - <code>{ch_id}</code> ({status} {'On' if mode == 'on' else 'Off'})</b></blockquote>\n"
...
async def force_sub_callback(client: Client, callback: CallbackQuery):
    ...
    if data == "fsub_add_channel":
        ...
        for ch_id in channels:
            try:
                ...
                logger.error(f"Failed to fetch chat {ch_id}: {e}")
                buttons.append([InlineKeyboardButton(text=f"⚠️ {ch_id} (Unavailable)", callback_data=f"rfs_ch_{ch_id}")])
        ...
        for ch_id in channels:
            try:
                ...
                status = "🟢" if mode == "on" else "🔴"
                title = f"{status} {chat.title}"
                buttons.append([InlineKeyboardButton(title, callback_data=f"fsub_toggle_single_{ch_id}")])
            except Exception as e:
                logger.error(f"Failed to fetch chat {ch_id}: {e}")
                buttons.append([InlineKeyboardButton(text=f"⚠️ {ch_id} (Unavailable)", callback_data=f"fsub_toggle_single_{ch_id}")])

        buttons.append([InlineKeyboardButton("Back ↔️", callback_data="fsub_back")])
        buttons.append([InlineKeyboardButton("Close ✖️", callback_data="close")])

        await temp.edit(
            "<blockquote><b>⚡ Select a channel to toggle its force-sub mode:</b></blockquote>",
            reply_markup=InlineKeyboardMarkup(buttons),
            disable_web_page_preview=True
        )
        await callback.answer()
    ...
    elif data.startswith("fsub_toggle_single_"):
        ...
        try:
            ...
            await callback.message.edit_text(
                ...
                f"<blockquote><b>Name:</b> <a href='https://t.me/{chat.username if chat.username else ch_id}'>{chat.title}</a></blockquote>\n"
            ...
            await callback.answer(f"Force-sub for channel {ch_id} {'enabled' if new_mode == 'on' else 'disabled'}")
        except Exception as e:
...
async def handle_channel_input(client: Client, message: Message):
    ...
    try:
        if state == "awaiting_add_channel_input":
            ...
            channel_ids_only = [cid for cid in all_channels]
            ...
            if member.status not in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
                await message.reply("<b>❌ Bot must be an admin in that channel.")
                await db.set_temp_state(chat_id, "")
                await show_force_sub_settings(client, chat_id)
                return
