...
async def show_force_sub_settings(client: Client, chat_id: int, message_id: int = None):
    ...
    settings_text = f"<b>›› Force-Sub Settings:</b>\n\n<b>Global Mode:</b> {'🟢 Enabled' if global_mode else '🔴 Disabled'}\n\n"
    channels = await db.show_channels()
    
    if not channels:
        settings_text += "<blockquote><i>No channels configured yet. Use 'Add Channels' to add a channel.</i></blockquote>"
    else:
        ...
        for ch_id in channels:
            try:
                ...
                settings_text += f"<blockquote><b>{status} <a href='{link}'>{chat.title}</a> - <code>{ch_id}</code></b></blockquote>\n"
            except Exception as e:
                logger.error(f"Failed to fetch chat {ch_id}: {e}")
                settings_text += f"<blockquote><b>⚠️ <code>{ch_id}</code> — <i>Unavailable</i></b></blockquote>\n"
    ...
    buttons = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton("Toggle Channel Mode", callback_data="fsub_toggle_mode"),
                InlineKeyboardButton("Toggle Global Mode", callback_data="fsub_toggle_global_mode")
            ],
            [
    ...
    if message_id:
        try:
            ...
            logger.info(f"Edited force-sub settings message in chat {chat_id}")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Sent force-sub settings photo message in chat {chat_id}")
        except Exception as e:
            logger.error(f"Failed to send photo message in chat {chat_id}: {e}")
            try:
                ...
                logger.info(f"Sent text-only force-sub settings message in chat {chat_id} as fallback")
            except Exception as e:
                logger.error(f"Failed to send text-only message in chat {chat_id}: {e}")
                await client.send_message(
                    chat_id=chat_id,
                    text=settings_text,
                    reply_markup=buttons,
                ...
                logger.info(f"Sent text-only force-sub settings message without effect in chat {chat_id}")
...
async def force_sub_callback(client: Client, callback: CallbackQuery):
    ...
    if data == "fsub_add_channel":
        ...
        await client.edit_message_text(
            chat_id=chat_id,
            message_id=message_id,
            text="<blockquote><b>Provide the channel ID or type '<code>all</code>' to remove all channels.</b></blockquote>",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("Back", callback_data="fsub_back"),
                    InlineKeyboardButton("Cancel", callback_data="fsub_cancel")
        ...
        temp = await callback.message.reply("<b><i>Fetching channels...</i></b>", quote=True)
        channels = await db.show_channels()

        if not channels:
            await temp.edit("<blockquote><b>❌ No force-sub channels found.</b></blockquote>")
            await callback.answer("No channels to toggle.")
            return
        ...
        buttons.append([InlineKeyboardButton("Close", callback_data="fsub_close")])
    ...
    elif data == "fsub_toggle_global_mode":
        if user_id != OWNER_ID:
            await callback.answer("Only the owner can toggle global mode!", show_alert=True)
            return
        current_mode = await db.get_force_sub_global_mode()
        new_mode = not current_mode
        await db.set_force_sub_global_mode(new_mode)
        status = "🟢 Enabled" if new_mode else "🔴 Disabled"
        await callback.message.edit_text(
            f"<blockquote><b>✅ Global force-sub mode updated:</b></blockquote>\n\n"
            f"<blockquote><b>Status:</b> {status}</blockquote>",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("Back", callback_data="fsub_back")]
            ]),
            parse_mode=ParseMode.HTML
        )
        await callback.answer(f"Global force-sub mode {'enabled' if new_mode else 'disabled'}.")
        logger.info(f"Global force-sub mode toggled to {new_mode} by user {user_id}")
...
async def toggle_channel_mode(client: Client, callback: CallbackQuery):
    ...
    try:
        ...
        await callback.answer("Failed to toggle channel mode.")
